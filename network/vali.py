...
inReader = csv.reader(open(
    "/Volumes/AnyShare/GroupDocuments.localized/ChallengeCup2020/ChallengeCup-master/network/input_test.csv", "r"))
for i in inReader:
    tmp = [float(j) for j in i]
    spec.append(tmp)

outReader = csv.reader(open(
    "/Volumes/AnyShare/GroupDocuments.localized/ChallengeCup2020/ChallengeCup-master/network/output_test.csv", "r"))
for i in outReader:
    tmp = [float(j) for j in i]
    res.append(tmp)
